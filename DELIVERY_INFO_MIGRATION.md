...
Mapping from the existing per-delivery-method columns in seven modules onto the
...
`PRTfullJSON.json`, `SRfullJSON.json`) — not from field names alone. Component types and stored
...
module names the same concept differently. `Driver Name` alone lives under seven
different column names across the seven forms. Because the grids are mutually
...
- **Need a name→id lookup:** the other 18 sources store a typed name string —
  every Self Pickup and 3rd Party column, plus GD/PP/PICK/PRT/SR Company Truck.
...
- **Need a plate→id lookup:** the other 17 sources store a free-typed plate
  string — every Self Pickup and 3rd Party column, plus PP `vehicle_no`,
  PRT `vehicle_no2` and SR `sr_vehicle_no` (all Company Truck, all plain inputs).
