    ...
    if (selectedRecords && selectedRecords.length > 0) {
      ...
      for (const ppItem of pickingPlanData) {
        ...
        try {
          ...
          if (ppItem.to_status !== "Created") {
            console.log(
              `Picking plan is not in Created status (current: ${ppItem.to_status}), skipping inventory reversal`
            );
          } else {
            try {
              for (const item of items) {
                ...
                for (const temp of temporaryData) {
                  // UOM Conversion
                  let altQty = parseFloat(temp.to_quantity);
                  let baseQty = altQty;
          ...
          // Update picking plan status
          await db.collection("picking_plan").doc(id).update({
            to_status: "Cancelled",
          ...
          if (ppItem.picking_status === "Created") {
            ...
            if (toResult.data && toResult.data.length > 0) {
              ...
              console.warn("Transfer order not found for picking plan:", id);
            }
          }

          for (const soItem of ppItem.so_id) {
            await db
