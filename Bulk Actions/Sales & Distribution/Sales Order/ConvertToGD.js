  ...
  try {
    ...
    if (selectedRecords && selectedRecords.length > 0) {
      selectedRecords = selectedRecords.filter(
        (item) =>
          item.so_status === "Issued" || item.so_status === "Processing",
      );
      ...
      await this.$confirm(
        `Only these sales order records available for conversion. Proceed?<br><br>
      ...
      if (selectedRecords.length > 0) {
        ...
        if (organizationId === "0") {
          ...
          if (response.data.length > 0) {
            let plants = new Set(
              selectedRecords.map((record) => record.plant_name.id),
            );
            ...
            if (plants.size > 1) {
              await this.$alert(
                "All selected sales orders must be from the same plant/organization to create a single goods delivery.",
                "Error",
                {
                  confirmButtonText: "OK",
                  type: "error",
            ...
            if (plants.size === 1) {
              ...
              if (plantID !== organizationId) {
                await this.triggerEvent("func_handleConvertGD", {
                  selectedRecords,
                  plantID,
