...
// Credits are read off the GROSS amount on purpose - a discount is a price
// concession, not a smaller top-up (same as SO, where so_discount never touches
...
if (reloadType === "Flex") {
  ...
  // Owned by onChange_ai_credit_plan. Read back, never recomputed here.
  credits = parseFloat(data.ai_credit_reload_amount) || 0;
}
...
// Written back only when this function actually changed them - echoing every
// keystroke back into the inputs fights the caret.
if (uomDefaulted) {
