// Fetch today's Bank Negara Malaysia rates, then apply this SO's currency rate.
...
(async () => {
  try {
    ...
    const soTotal = this.getValue("so_total");
    if (soTotal === undefined || soTotal === 0) {
      await this.setData({ myr_total_amount: 0.0 });
    } else {
      await this.setData({
        myr_total_amount: parseFloat((rate * soTotal).toFixed(4)),
      });
    }
