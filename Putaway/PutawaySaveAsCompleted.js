  ...
  for (const item of tablePutawayItem) {
    ...
    if (
      ...
      if (trimmedSerialNumbers.length > 0) {
        ...
        console.log("Serial Numbers:", trimmedSerialNumbers);

        putawayRecord.serial_numbers = JSON.stringify(
          trimmedSerialNumbers.join("\n"),
        );

        console.log("Serial Numbers:", putawayRecord.serial_numbers);
  ...
  try {
    ...
    const initializeBalanceData = (matData) => {
      ...
      const newBalance = {
        ...
        material_uom: matData.material_uom || "",
      };
    ...
    for (const key in balanceUpdates) {
      ...
      if (balanceData.data.length > 0) {
        ...
        const updatedBalance = {
          ...
          material_uom: record.material_uom || "",
          ...(collection === "item_batch_balance" && {
        ...
        if (update.movementType === "TRANSFER") {
          ...
          newBalance.unrestricted_qty = Math.max(
            0,
            ...
            if (resItem && resItem.data.length > 0) {
              ...
              if (!isSerializedItem) {
                ...
                if (isSameLocation) {
                  ...
                  console.log(
                    "Same location detected - handling as category transfer",
                    {
                  ...
                  console.log("Category transfer quantities", {
                    ...
                    unrestricted_qty:
                      balanceUpdates[transferKey].unrestricted_qty,
                    qualityinsp_qty:
                      balanceUpdates[transferKey].qualityinsp_qty,
                  ...
                  balanceUpdates[inKey].reserved_qty +=
                    inCategories.reserved_qty;
...
(async () => {
  try {
    ...
    console.log("Table putaway item", data.table_putaway_item);
