...
const processInventoryMovementandBalanceTable = async (
  ...
  try {
    for (const mat of updatedItems) {
      if (mat.is_split === "Yes" && mat.parent_or_child === "Parent") {
        continue;
      } else {
        if (mat.item_code) {
...
const updateGoodsReceiving = async (grId, isAutoCompleteGR, organizationId) => {
  try {
    ...
    // No QC, gr directly completed and putaway = completed
    ...
    // Have QC, so check all QC status
    else if (resQI && resQI.data.length > 0) {
      const unCompletedQC = resQI.data.filter(
        (qi) => qi.receiving_insp_status !== "Completed"
      );

      // Some QC is not completed
      if (unCompletedQC.length > 0) {
        isGRCompleted = "No";
        await db.collection("goods_receiving").doc(grId).update({
          putaway_status: "In Progress",
        });
      }
      // All QC is completed, then check QC putaway status
      else {
        for (const qi of resQI.data) {
          const resQIPutaway = await db
            .collection("transfer_order_putaway")
            .where({ gr_no: grId, qi_id: qi.id })
            .get();

          // If the QC has putaway
          if (resQIPutaway && resQIPutaway.data.length > 0) {
            const unCompletedPutaway = resQIPutaway.data.filter(
              (qi) => qi.to_status !== "Completed"
            );

            // Putaway status is not completed
            if (unCompletedPutaway.length > 0) {
              isGRCompleted = "No";
              await db.collection("goods_receiving").doc(grId).update({
                putaway_status: "In Progress",
              });
...
(async () => {
  try {
    ...
    const { updatedItems, errors } = await validateAndUpdateLineStatuses(
    ...
    if (
      ...
      const incompleteItems = updatedItems
        .map((item, index) => ({
          ...
          qtyToPutaway: parseFloat(item.pending_process_qty) || 0,
