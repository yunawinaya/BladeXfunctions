const setPutawayItemData = async (
    ...
    return {
      ...
      putaway_qty: isSplit === "No" && parentOrChild === "Parent"  ? itemData.putaway_qty : qtyToPutaway,
      ...
      for (const [index, paItem] of tablePutaway.entries()) {
        if (paItem.is_split === "Yes" && paItem.parent_or_child === "Parent") {
          ...
          if (paItem.is_serialized_item === 1) {
            this.disabled([`table_putaway_item.${index}.select_serial_number`, `table_putaway_item.${index}.putaway_qty`], true);
          }
          ...
          if (paItem.is_serialized_item === 1) {
            this.disabled([`table_putaway_item.${index}.select_serial_number`, `table_putaway_item.${index}.putaway_qty`], false);
          }
        }
      }
