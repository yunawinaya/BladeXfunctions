# Before & After: Performance Optimization for Picking Plan

## The Problem

> "if we have 50 data, the allocation process will takes super long"

**You were absolutely right!** The original code had the same severe performance issues as the Goods Delivery module.
...
this.setData({ [`table_to.${index}.material_id`]: materialId });
this.setData({ [`table_to.${index}.material_name`]: itemName });
this.setData({ [`table_to.${index}.to_material_desc`]: desc });
...
tableToArray[index] = {
  ...
  to_material_desc: desc,
  // ... all 20 fields at once
};

await this.setData({ table_to: tableToArray }); // Single call!
```
...
this.setData({ [`table_to.${i}.field1`]: value1 });
this.setData({ [`table_to.${i}.field2`]: value2 });
// ... 20 fields × 50 items
```
...
await this.setData({ table_to: array });
```
...
- ✅ Picking Plan specific logic (planned_qty tracking)
...
### Before:
- User selects 50 items from sales orders for Picking Plan
- Clicks "Add to Picking Plan"
...
```
materialID mat001
DEBUG - Material mat001:...
materialID mat002
DEBUG - Material mat002:...
materialID mat003
DEBUG - Material mat003:...
(repeats 50 times)
...
```bash
# Backup original
mv "Picking Plan/PPaddBatchLineItem.js" "Picking Plan/PPaddBatchLineItem.js.backup"

# Deploy optimized version
mv "Picking Plan/PPaddBatchLineItem_OPTIMIZED.js" "Picking Plan/PPaddBatchLineItem.js"
```
...
## 🆕 Picking Plan Specific Features Preserved

The optimization maintains all Picking Plan specific logic:

**1. Planned Quantity Tracking**
```javascript
// Correctly uses planned_qty instead of delivered_qty
deliveredQtyFromSource: parseFloat(soItem.planned_qty || 0)
```

**2. Multi-Customer Support**
```javascript
// Supports multiple customers in one Picking Plan
customer_name: allCustomerIds
```

**3. Sales Order Integration**
```javascript
// Picking after: "Sales Order" (not "Goods Delivery")
.where({ plant_id: plantId, picking_after: "Sales Order" })
```

**4. All Table Field Names**
- Uses `table_to` instead of `table_gd`
- Uses `to_qty` instead of `gd_qty`
- Uses `to_delivered_qty` instead of `gd_delivered_qty`
- All field mappings preserved correctly
...
The code is now production-ready for high-volume Picking Plan operations! 🚀
