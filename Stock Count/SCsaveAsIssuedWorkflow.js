// Stock Count — Save as Issued (workflow trigger)
// Calls combined SCsaveWorkflow.json with saveAs="Issued".
...
(async () => {
  ...
  try {
    ...
    const res = await runSCWorkflow("Issued", data);
    handleResult(res, "Saved as Issued");
  } catch (error) {
    this.hideLoading();
    console.error(error);
    this.$message.error((error && (error.message || error.msg)) || "Failed to save Issued");
  }
})();
