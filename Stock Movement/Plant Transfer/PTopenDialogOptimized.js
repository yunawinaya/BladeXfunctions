(async () => {
  ...
  try {
    ...
    const buildHandlingUnits = (
      ...
      // hu_select on matching header rows. Map lookup replaces O(T*N) linear scan.
      ...
      if (parsedTempHu.length > 0) {
        ...
        // PT enforces NO_SPLIT: set hu_select = 1 on headers whose HU has any
    ...
    // Hide category-from/to + serial column. hu_select stays visible — PT enforces
    ...
    //       item_balance display so PT doesn't pick stock already committed to GD.
    ...
    // in buildHuQtyMap and to hide reserved HUs from the HU tab.
    ...
    // categories aren't transferable via PT).
    ...
    // reservations (bucket-shifted to reserved_qty on save), so only
    // unreserved HU qty needs deducting here to isolate truly-loose stock.
    ...
    if (hasHu) {
      ...
      // Batch all disables into a single call — per-row calls froze the UI at
    ...
    console.error("Error in PT inventory dialog:", error);
  } finally {
    this.hideLoading();
  }
})();
