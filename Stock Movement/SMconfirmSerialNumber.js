(async () => {
  try {
    ...
    await this.setData({
      [`stock_movement.${dialogData.row_index}.serial_number_data`]:
        JSON.stringify(mappedData),
    });

    await this.setData({
      [`stock_movement.${dialogData.row_index}.received_quantity`]:
        dialogData.new_rows,
    });

    if (dialogData.new_rows > 0) {
      this.setData({
        [`stock_movement.${dialogData.row_index}.is_serial_allocated`]: 1,
      });
    }
