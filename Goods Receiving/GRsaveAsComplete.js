...
const addInventory = async (
  ...
  const createInspectionLot = async (
    ...
    try {
      const inspPrefixID = await getDefaultDocFormat(
  ...
  try {
    ...
    if (pageStatus === "Edit") {
      ...
      const response = await db
        ...
        .doc(grID)
        .update(entry);
      result = response.data.modifiedResults[0];
    }
    ...
    await createSerialNumberRecord(result);
